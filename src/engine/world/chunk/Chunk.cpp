...
Block Chunk::getBlock(uint16_t index) const {
...
void Chunk::setBlock(uint16_t index, Block block) {
    this->blocks[index] = block;
}

uint16_t Chunk::getIndex(Vector3u8 pos) {
    return (pos.z * CHUNK_SIZE2) + (pos.y * CHUNK_SIZE) + pos.x;
}
bool Chunk::outOfBounds(uint16_t index) {
